...
def initial_state():
    ...
    # returning the perceptron as a list
    return []
...
def predict(state, kernel, x_i):
    ...
    ker_pred = 0
    for beta_i, x in state:
        ker_pred += beta_i * kernel(x_i, x)   # x_i training examples and x testing samples
    return sign(ker_pred)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    beta_i_new = learning_rate * (y_i - predict(state, kernel, x_i))    # derived in lecture through induction
    state.append((beta_i_new, x_i))
